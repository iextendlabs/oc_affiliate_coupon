...
class ControllerMarketingCoupon extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$coupon_id = $this->model_marketing_coupon->getLastCoupon();
			if($this->request->post['affiliate_id'] != 0){
				$this->model_marketing_coupon->addCouponAffiliate($this->request->post, $coupon_id);
			}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_marketing_coupon->deleteCouponAffiliate($this->request->get['coupon_id']);
				
			if($this->request->post['affiliate_id'] != 0){
				$this->model_marketing_coupon->editCouponAffiliate($this->request->get['coupon_id'], $this->request->post);
			}
			$this->session->data['success'] = $this->language->get('text_success');
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $coupon_id) {
				...
				$this->model_marketing_coupon->deleteCouponAffiliate($coupon_id);
			}
	...
	protected function getForm() {
		...
		$data['column_affliate'] = $this->language->get('column_affliate');
		$data['column_commission'] = $this->language->get('column_commission');
		$data['text_no_affiliate'] = $this->language->get('text_no_affiliate');
		...
		if (isset($this->request->get['coupon_id']) && (!$this->request->server['REQUEST_METHOD'] != 'POST')) {
			...
			$coupon_affiliate_info = $this->model_marketing_coupon->getCouponAffiliate($this->request->get['coupon_id']);
		}

		if (isset($this->request->post['affiliate_id'])) {
			$data['affiliate_id'] = $this->request->post['affiliate_id'];
		} elseif (!empty($coupon_affiliate_info)) {
			$data['affiliate_id'] = $coupon_affiliate_info['affiliate_id'];
		} else {
			$data['affiliate_id'] = '';
		}

		if (isset($this->request->post['affiliate_commission'])) {
			$data['affiliate_commission'] = $this->request->post['affiliate_commission'];
		} elseif (!empty($coupon_affiliate_info)) {
			$data['affiliate_commission'] = $coupon_affiliate_info['affiliate_commission'];
		} else {
		...
		$data['customer_affiliates'] = $this->model_marketing_coupon->getCustomerAffiliates();
