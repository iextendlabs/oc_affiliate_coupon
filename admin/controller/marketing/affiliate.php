...
class ControllerMarketingAffiliate extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->get['affiliate_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			...
			$data['coupons'] = $this->model_marketing_affiliate->getAffiliateCoupons($affiliate_id);
			...
			$data['filter_report_action'] = str_replace('&amp;', '&', $this->url->link('marketing/affiliate/edit', 'token=' . $this->session->data['token'] . '&affiliate_id=' . $affiliate_id));
			...
			foreach ($order_status_ids as $order_status_id) {
				...
				$orders = $this->model_marketing_affiliate->getCustomerCommissionTransactions($affiliate_id, $order_status_id, $filter_month, $filter_year);
				...
				if($orders){
					...
					// get order subtotal
					$order_subtotal = $this->model_marketing_affiliate->getOrderSubTotal($order_id);
					...
					$data[$commission_type] = $this->currency->format($this->model_marketing_affiliate->getCustomerTransactionTotal($affiliate_id, $order_status_id), $this->config->get('config_currency'));
