		...
		if ($code) {
			...
			if ($coupon_query->num_rows) {
				$coupon_total = $this->getTotalCouponHistoriesByCoupon($code);
					...
					$coupon_affiliate = $this->db->query("SELECT affiliate_id,affiliate_commission FROM ".DB_PREFIX."coupon_affiliate  WHERE coupon_id=".$coupon_query->row['coupon_id']);
					...
					if($coupon_affiliate->num_rows){
						...
						if($affiliate_id !=0){
							...
							$commission = ($coupon_affiliate->row['affiliate_commission'] / 100 ) * $sub_total->row['value'] ;
